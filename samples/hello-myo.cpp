			...
			if (boundsSet == 0) {
				...
				if (gameRunning) {
					if (clock.getElapsedTime ().asSeconds () > fruitTime) {
						...
						Fruit *newFruit = new Fruit (isBomb, false, false, type, isBomb ? bombTexture1 : (fruitTextures [type]));
					...
					for (int i = 0; i < fruit.size (); i++) {
						...
						for (int j = 0; j < pointers.size (); j++) {
							if (pointers [j] && !fruit [i]->isDead () && !fruit [i]->isSplit () && abs (fruit [i]->getPosition ().x - pointers [j]->getX ()) < 37.5 && abs (fruit [i]->getPosition ().y - pointers [j]->getY ()) < 37.5) {
								Fruit *newFruit = new Fruit (fruit [i]->isBomb (), false, true, fruit [i]->getType (), fruit [i]->isBomb () ? bombTexture2 : (fruitTextures [fruit [i]->getType () + 1]), fruit [i]->getYSpeed () - 0.5);
								newFruit->setPosition (fruit [i]->getPosition ());
								newFruit->setOrigin (newFruit->getOrigin ().x + 37.5, newFruit->getOrigin ().y + 37.5);
								fruit.push_back (newFruit);
								Fruit *newFruit2 = new Fruit (fruit [i]->isBomb (), false, true, fruit [i]->getType (), fruit [i]->isBomb () ? bombTexture3 : (fruitTextures [fruit [i]->getType () + 2]), fruit [i]->getYSpeed () - 0.5);
						...
						if (!fruit [i]->isDead () && !fruit [i]->isSplit () && fruit [i]->getPosition ().y > screenHeight && fruit [i]->getYSpeed () > 0) {
							if (!fruit [i]->isBomb ())
								fails++;
							fruit [i]->setDead (true);
						}
